...
class SkillProcessManager:
    ...
    async def start(
        ...
        *,
        ...
        # Serialize creation with lifecycle cleanup.  Otherwise a concurrent
        # session cleanup could observe no record while a sidecar lease was
        ...
        async with self._lock:
            ...
            try:
                lease, _opened = await open_isolated_process_lease(
                ...
                started = await start_isolated_process_lease(
                    lease,
                    op_id=start_op_id,
                )
                ...
                if lease is not None:
                    try:
                        ...
                        # failed.  Keep an internal, unreturned record so
                        # root/session/shutdown cleanup can retry the exact
                        # two-phase close transaction instead of orphaning the
                        # lease until its TTL and blocking the single worker
                        # slot meanwhile.
            ...
            process_id = self._new_process_id()
            while process_id in self._records or process_id in self._closed:
                process_id = self._new_process_id()
    ...
    async def forget_closed(self, record: _ManagedProcess) -> None:
        async with self._lock:
            current = self._records.get(record.process_id)
            if current is record:
                self._records.pop(record.process_id, None)
                self._closed[record.process_id] = record.owner
                while len(self._closed) > 2_048:
                    self._closed.pop(next(iter(self._closed)), None)

    async def forget_lost(self, record: _ManagedProcess) -> None:
        """Drop a client record after the executor proves the lease is gone."""
    ...
    async def close_explicit(
        ...
        async with self._lock:
            ...
            if record.closing:
                raise IsolatedSkillExecutorError(
                    "process_closing",
                    "The persistent Skill process is already closing.",
                )
            if not _context_permits_record(context, record.authority):
                raise IsolatedSkillExecutorError(
                    "process_authority_mismatch",
        ...
        async with record.lock:
            try:
                ...
                if (
                    ...
                    await self.forget_lost(record)
                else:
                    async with self._lock:
                        if self._records.get(process_id) is record:
                            record.closing = False
                raise
        await self.forget_closed(record)
    ...
    async def _cleanup_matching(
        ...
        *,
        ...
        outcomes = await asyncio.gather(*(close_one(record) for record in records))
        async with self._lock:
            for record, _error_code, terminal in outcomes:
                if terminal or _error_code is None:
                    if self._records.get(record.process_id) is record:
                        self._records.pop(record.process_id, None)
                        self._closed[record.process_id] = record.owner
                elif self._records.get(record.process_id) is record:
                    # A transient close/sync/CAS failure retains the opaque
                    # runtime record so session cleanup or shutdown can retry
                    # the exact pending transaction instead of orphaning it.
                    record.closing = False
            failed_owners = {
                record.owner
        ...
        return {
            ...
            "reason": reason,
            "matched": len(records),
            "closed": len(records) - len(failures),
    ...
    try:
        ...
        async with record.lock:
            ...
            if operation == "write":
                ...
                response = await write_isolated_process_stdin(
                    record.lease,
                    content,
                    op_id=_operation_uuid(context, "write"),
                )
            elif operation == "stdin_close":
                response = await close_isolated_process_stdin(
                    record.lease,
                    op_id=_operation_uuid(context, "stdin-close"),
                )
            elif operation == "read":
                response = await read_isolated_process_output(
                    ...
                    max_bytes=max_bytes,
                    wait_ms=wait_ms,
                    op_id=_operation_uuid(context, "read"),
                )
            elif operation == "call":
                ...
                response = await call_isolated_process_instance(
                    record.lease,
                    method_name=selected_method,
                    method_args=method_args,
                    method_kwargs=method_kwargs,
                    op_id=_operation_uuid(context, "call"),
                )
            elif operation == "sync":
                ...
                response = await signal_isolated_process(
                    record.lease,
                    str(signal or ""),
                    op_id=_operation_uuid(context, "signal"),
                )
        ...
        if code in {
            ...
            if active_record is not None:
                await _MANAGER.forget_lost(active_record)
            code = "lease_lost"
