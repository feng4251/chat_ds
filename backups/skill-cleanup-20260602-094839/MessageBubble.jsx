...
import { FiChevronDown, FiChevronRight, FiCopy, FiCheck, FiZap, FiCpu, FiX } from 'react-icons/fi'
...
function CodeBlock({ children }) {
  ...
  return (
    <div className="relative group my-3 rounded-xl overflow-hidden border border-slate-800/40 shadow-sm bg-slate-950">
      ...
      <pre className="bg-transparent px-4 py-3 overflow-x-auto text-[12.5px] leading-relaxed text-slate-100 m-0">
        {children}
      </pre>
    </div>
  )
...
const mdComponents = {
  ...
  code: ({ className, children, ...props }) => {
    ...
    return (
      <code
        className="font-mono text-[0.9em] text-rose-600"
        {...props}
      >
        {children}
      </code>
  ...
  h1: ({ children }) => (
    <h1 className="text-xl font-semibold mt-4 mb-2 tracking-tight text-slate-900">
      {children}
    </h1>
  ),
  h2: ({ children }) => (
    <h2 className="text-lg font-semibold mt-3.5 mb-2 tracking-tight text-slate-900">
      {children}
    </h2>
  ),
  h3: ({ children }) => (
    <h3 className="text-base font-semibold mt-3 mb-1.5 tracking-tight text-slate-900">
      {children}
    </h3>
  ),
  h4: ({ children }) => (
...
export function MessageBubble({ msg }) {
  ...
  const showReasoning =
    userShowReasoning === null ? autoShowReasoning : userShowReasoning
  ...
  const hasSkill = !!msg.skill_chain
  const hasReasoning = !!msg.reasoning
  const skillLines = hasSkill ? msg.skill_chain.split('\n') : []
        ...
        <div className="flex-1 min-w-0">
          {hasSkill && (
            <div className="mb-1.5">
              ...
              >
                {showSkill ? <FiChevronDown size={12} /> : <FiChevronRight size={12} />}
                <FiZap size={11} />
                技能执行
          ...
          {msg.content && !msg.streaming && (
            <button
              onClick={handleCopy}
              className="mt-1.5 text-xs text-slate-400 hover:text-slate-700 flex items-center gap-1 transition"
            >
