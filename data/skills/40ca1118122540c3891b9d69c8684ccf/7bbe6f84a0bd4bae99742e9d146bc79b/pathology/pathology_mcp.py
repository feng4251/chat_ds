...
(`~/.config/claude/claude_desktop_config.json` or equivalent):

    {
      "mcpServers": {
        "pathology": {
          "command": "python",
          "args": ["/abs/path/to/pathology_mcp.py"],
          "env": {
...
API_URL = os.environ.get("PATHOLOGY_API_URL", "http://10.10.132.128:18018").rstrip("/")
...
def _summarize(resp: dict) -> str:
    ...
    qc = resp.get("qc") or {}
    lines.append(
        f"QC: pass={qc.get('qc_pass')} tissue_ratio={qc.get('tissue_ratio')} "
        f"blur={qc.get('blur_score')}"
    )
