...
> 当前建议：SearXNG 部署在本机 `10.10.132.126`，`chat_ds` 远端 `10.10.130.178` 只需要访问本机 SearXNG HTTP 服务，不需要直接放开所有搜索引擎出站。
...
当前 SearXNG 配置启用 Bing 作为主搜索引擎：

| 源 | 目的域名 | 协议/端口 | 用途 | 当前本机探测 |
|---|---|---:|---|---|
| `10.10.132.126` | `www.bing.com` | HTTPS `443` | Bing 主搜索，SearXNG 当前实际使用 | 可达 |
...
- 远端 `10.10.130.178`：设置 `SEARXNG_BASE_URL=http://10.10.132.126:8088`，`WEB_SEARCH_PROVIDERS=searxng,ddg`。
